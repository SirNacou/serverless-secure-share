...
export const handler = async (event) => {
    try {
        if (!BUCKET_NAME || !TABLE_NAME) {
            return {
                statusCode: 500,
                body: JSON.stringify({ error: "System configuration error: missing environment variables." }),
            };
        }
        ...
        if (!filename || !contentType) {
            return {
                statusCode: 400,
                body: JSON.stringify({ error: "Validation failed: 'filename' and 'contentType' are required." }),
            };
        }
        ...
        const uploadUrl = await getSignedUrl(s3Client, s3Command, { expiresIn: 900 });
